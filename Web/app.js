...
const DEFAULT_PREFERENCES = { autoplayNext:true, autoplayPreviews:true, episodeAlerts:false, maturity:"18+", language:"English", familySafe:false, blockScary:false, searchEnabled:true, moviesEnabled:true, seriesEnabled:true, introEnabled:true, playerProvider:"vidsrc" };
const PLAYER_PROVIDERS = Object.freeze(["vidsrc", "2embed"]);
...
function selectedPlayerProvider() { const provider = currentPreferences().playerProvider; return PLAYER_PROVIDERS.includes(provider) ? provider : "vidsrc"; }
...
function playerURL(item, progress = 0) {
  ...
  else {    const base = item.type === "movie" ? `movie/${item.id}` : `tv/${item.id}/${item.season || 1}/${item.episode || 1}`;
...
if ("serviceWorker" in navigator) {
  navigator.serviceWorker.register("/service-worker.js?v=220").catch(() => { /* The app keeps working from the network when registration fails. */ });
}
