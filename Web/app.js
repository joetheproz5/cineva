...
function renderLaunchIntro() {
  ...
  const fill = () => {
    ...
    overlay.innerHTML = `<div class="seven-intro-scene" aria-hidden="true"><div class="seven-intro-aperture"></div><div class="seven-intro-beam"></div><div class="seven-intro-lockup"><div class="seven-intro-logo-wrap">${shards.map(piece => `<img class="seven-intro-piece ${piece}" src="${logo.src}" alt="">`).join("")}<span class="seven-intro-polish"></span></div><span class="seven-intro-tagline">YOUR NEXT STORY STARTS HERE</span></div></div>`;
    clearTimeout(state.introTimer);
...
if ("serviceWorker" in navigator) {
  navigator.serviceWorker.register("service-worker.js?v=231").catch(() => { /* The app keeps working from the network when registration fails. */ });
}
