...
function renderPlayer() {
  ...
  document.querySelector("[data-provider-toggle]")?.addEventListener("click", async () => { const order = ["vidlink", "vidking", "vidsrc", "2embed"], next = order[(order.indexOf(currentPreferences().playerProvider || "vidking") + 1) % order.length]; updateCurrentPreferences({ playerProvider: next }); await saveAccount(); render(); });
  renderPartyPanel();
