...
function header() {
  ...
  const account = state.user ? `<button class="account signed-in" data-account title="Account">${profileAvatar(profile)}<i class="account-caret" aria-hidden="true">▾</i></button>` : `<button class="account account-guest" data-auth aria-label="Sign in or create an account" title="Sign in"><svg class="account-guest-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.25"/><path d="M5.5 20c.65-3.55 2.82-5.4 6.5-5.4s5.85 1.85 6.5 5.4"/></svg><b>${t("Sign in")}</b></button>`;
...
async function repairContinuePosters(items) {
  ...
  await Promise.all(titles.map(async ({ type, id }) => {
    ...
    const changed = [];
    Object.keys(localStorage).filter(key => key.startsWith(`seven-progress-${activeProfileId()}-`)).forEach(storageKey => { try { const record = JSON.parse(localStorage.getItem(storageKey) || "{}"); if (record.type !== type || Number(record.id) !== id || record.posterPath === posterPath) return; record.posterPath = posterPath; localStorage.setItem(storageKey, JSON.stringify(record)); changed.push({ storageKey, record }); } catch {} });
