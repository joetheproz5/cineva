...
const DEFAULT_PREFERENCES = { autoplayNext:true, autoplayPreviews:true, episodeAlerts:false, maturity:"18+", language:"English", familySafe:false, blockScary:false, searchEnabled:true, moviesEnabled:true, seriesEnabled:true, introEnabled:true, playerProvider:"vidking" };
...
function renderHome() {
  ...
  bindCommon(); initCoverflow(); scheduleHero(); syncHeaderScroll();
...
function playerURL(item, progress = 0) {
  const preferences = currentPreferences(), provider = preferences.playerProvider || "vidking";
  ...
  if (provider === "vidlink") {
    ...
    return `https://vidlink.net/${base}?${new URLSearchParams({ color:"b20710", autoPlay:"true", nextEpisode:String(preferences.autoplayNext !== false), episodeSelector:"true", ...(progress > 0 ? { progress:String(Math.floor(progress)) } : {}) })}`;
  }
...
function renderPlayer() {
  ...
  document.querySelector("[data-provider-toggle]")?.addEventListener("click", async () => { const order = ["vidking", "vidlink", "vidsrc", "2embed"], next = order[(order.indexOf(currentPreferences().playerProvider || "vidking") + 1) % order.length]; updateCurrentPreferences({ playerProvider: next }); await saveAccount(); render(); });
  renderPartyPanel();
