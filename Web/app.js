...
const UI_STRINGS = {
  Arabic: {
    ...
    "Browse":"تصفح", "Watching now":"يشاهد الآن",
    ...
    "Kids profile":"ملف أطفال", "Screen time limit":"حد وقت الشاشة", "Daily limit":"الحد اليومي",
  ...
  French: {
    ...
    "Browse":"Parcourir", "Watching now":"En cours",
    ...
    "Kids profile":"Profil enfant", "Screen time limit":"Limite de temps d'écran", "Daily limit":"Limite quotidienne",
...
const DEFAULT_PREFERENCES = { autoplayNext:true, autoplayPreviews:true, episodeAlerts:false, maturity:"18+", language:"English", familySafe:false, blockScary:false, searchEnabled:true, moviesEnabled:true, seriesEnabled:true, introEnabled:true, adblock:true, playerProvider:"vidlink" };
...
function header() {
  ...
  const account = state.user ? `<button class="account signed-in" data-account title="Account">${profileAvatar(profile)}<i class="account-caret" aria-hidden="true">▾</i></button>` : `<button class="account" data-auth aria-label="Open account"><span>◉</span><b>${t("Account")}</b></button>`;
...
function renderHome() {
  ...
  if (continuing.length) void repairContinueSeriesPosters(continuing);
...
function heroQueue() {
  ...
  if (count < 2) return "";
  return `<div class="hero-queue" aria-label="More featured titles">${[1, 2, 3].filter(offset => offset < count).map(offset => { const index = (state.featuredIndex + offset) % count, item = state.featuredPool[index]; return `<button data-hero-index="${index}" aria-label="Feature ${escapeHTML(titleOf(item))}"><img src="${posterOf(item)}" alt="" loading="eager"><span><small>UP NEXT</small><b>${escapeHTML(titleOf(item))}</b></span></button>`; }).join("")}</div>`;
}
function featuredMarkup(f = state.featured) {
  ...
  return `<div class="home-hero-backdrop" style="background-image:url('${escapeHTML(backdrop)}')"></div><div class="home-hero-shade"></div><div class="home-hero-content"><span class="hero-kicker"><i></i> SEVEN PREMIERE</span><h1>${escapeHTML(titleOf(f))}</h1><div class="hero-meta"><strong>${score ? `${Math.round(score * 10)}% match` : "Featured"}</strong><span>${yearOf(f) || "New"}</span><span>${type === "tv" ? "Series" : "Movie"}</span><span class="hero-rating">${type === "tv" ? "TV" : "FILM"}</span></div><p>${escapeHTML(description)}</p><div class="hero-actions"><button class="hero-play" data-open="${type}:${f.id}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5v14l11-7z"/></svg>${t("Watch now")}</button><button class="hero-info" data-open="${type}:${f.id}"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 11v6m0-9h.01"/></svg>More info</button></div></div>${heroQueue()}<div class="hero-controls"><button data-hero-prev aria-label="Previous featured title">‹</button><span>${String(state.featuredIndex + 1).padStart(2, "0")} <i></i> ${String(Math.max(1, state.featuredPool.length)).padStart(2, "0")}</span><button data-hero-next aria-label="Next featured title">›</button></div>`;
}
function bindHeroControls() {
  ...
  featured.querySelectorAll("[data-open]").forEach(button => button.onclick = () => { const [type, id] = button.dataset.open.split(":"); openItem(type, Number(id)); });
}
...
function rail(name, items) { const visible = name === "My List" ? items : items.filter(contentAllowed); if (!visible.length) return ""; const key = `${state.route}:${name}`, displayName = name === "My List" ? "Favourites" : name, ranked = name === "Trending now"; state.explorable ||= {}; state.explorable[key] = { name:displayName, items:visible, fromRoute:state.route }; return `<section class="rail ${ranked ? "ranked-rail" : ""}"><div class="rail-title"><div>${railTitle(displayName)}<small>${ranked ? "The titles everyone is watching" : "Curated for your screen"}</small></div><button class="explore-all" data-explore="${escapeHTML(key)}">View all <b>›</b></button></div><div class="cards">${visible.map((item, index) => card(item, ranked ? index + 1 : 0)).join("")}</div></section>`; }
...
function card(item, rank = 0) { return `<button class="card ${rank ? "ranked-card" : ""}" data-open="${item.type}:${item.id}">${rank ? `<em class="card-rank">${rank}</em>` : ""}<span class="poster-wrap"><img src="${posterOf(item)}" alt="" loading="lazy"><span class="card-shade"></span><i>${item.type === "tv" ? "SERIES" : "MOVIE"}</i>${item.vote_average ? `<em class="card-score">★ ${item.vote_average.toFixed(1)}</em>` : ""}<strong class="card-play" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg></strong></span><span class="card-copy"><b>${escapeHTML(titleOf(item))}</b><small>${yearOf(item) || "New"} · ${item.type === "tv" ? "Series" : "Movie"}</small></span></button>`; }
...
function catalogSeriesPoster(id) { for (const items of Object.values(state.catalog || {})) { const match = items.find(item => contentType(item) === "tv" && Number(item.id) === Number(id) && item.poster_path); if (match) return match.poster_path; } return null; }
async function repairContinueSeriesPosters(items) {
  const series = [...new Set(items.filter(item => item.type === "tv").map(item => Number(item.id)))].filter(id => id && !continuePosterRepairs.has(id));
  await Promise.all(series.map(async id => {
    continuePosterRepairs.add(id);
    let posterPath = catalogSeriesPoster(id);
    if (!posterPath) { try { posterPath = (await api(`tv/${id}`)).poster_path; } catch { continuePosterRepairs.delete(id); return; } }
    if (!posterPath) { continuePosterRepairs.delete(id); return; }
    const changed = [];
    Object.keys(localStorage).filter(key => key.startsWith(`seven-progress-${activeProfileId()}-`)).forEach(key => { try { const record = JSON.parse(localStorage.getItem(key) || "{}"); if (record.type !== "tv" || Number(record.id) !== id || record.posterPath === posterPath) return; record.posterPath = posterPath; localStorage.setItem(key, JSON.stringify(record)); changed.push({ key, record }); } catch {} });
    document.querySelectorAll(`[data-continue^="tv:${id}:"] img`).forEach(image => { image.src = `${TMDB_IMAGE}${posterPath}`; });
    if (state.session && changed.length) await Promise.allSettled(changed.map(({ key, record }) => localAPI("/api/account/progress", { method:"POST", headers:{ "Content-Type":"application/json", ...authorizedHeaders() }, body:JSON.stringify({ content_key:key, content_type:"tv", tmdb_id:id, season:record.season || null, episode:record.episode || null, title:record.title || "Untitled", poster_path:posterPath, progress_seconds:Number(record.currentTime) || 0, duration_seconds:Number(record.duration) || 0, is_watched:Boolean(record.watched || Number(record.progress) >= 90), last_watched_at:record.lastWatchedAt || new Date().toISOString() }) })));
  }));
}
...
function renderSeries() {
  ...
  app.innerHTML = `${header()}<main class="series-page"><section class="series-hero"><div class="series-hero-backdrop" style="background-image:url('${backdrop}')"></div><div class="series-hero-shade"></div><button class="series-back" data-home>‹ <span>Back to browse</span></button><div class="series-hero-content"><img class="series-poster" src="${posterOf(s)}" alt="${escapeHTML(titleOf(s))}"><div class="series-hero-copy"><span class="series-eyebrow">SEVEN ORIGINAL SERIES</span><h1>${escapeHTML(titleOf(s))}</h1><div class="series-meta"><span>${yearOf(s)}</span><span>${s.number_of_seasons || seasons.length} seasons</span><span class="series-score">★ ${s.vote_average ? s.vote_average.toFixed(1) : "New"}</span></div><p>${escapeHTML(s.overview || "Discover the story, meet the characters, and start watching on SEVEN.")}</p><div class="actions">${action}<button class="secondary list-action ${isInMyList(s) ? "saved" : ""}" data-toggle-my-list>${isInMyList(s) ? "✓ In My List" : "+ My List"}</button>${trailerAction()}${hideAction(s)}</div></div></div></section><div class="series-shell"><section class="series-dashboard"><div class="series-progress-card"><div><span class="series-section-label">YOUR PROGRESS</span><strong>${tracking.watched} <small>of ${episodeTotal} episodes</small></strong><p>${progress ? "Keep going — your next episode is ready." : "Start the series and your progress will appear here."}</p></div><span class="series-progress-percent">${progress}%</span><i><em style="width:${progress}%"></em></i></div>${upNextCard}</section><section class="episode-section series-episodes"><div class="series-section-head"><div><span class="series-section-label">EPISODE GUIDE</span><h2>Season ${state.selectedSeason}</h2></div><label class="season-select"><span>Jump to</span><select id="season-selector" aria-label="Choose season">${seasons.map(x => `<option value="${x.season_number}" ${x.season_number === state.selectedSeason ? "selected" : ""}>Season ${x.season_number}</option>`).join("")}</select></label></div><div id="episodes-list" class="series-episode-grid">${episodeRows || '<p class="episode-empty">No episodes are available for this season.</p>'}</div></section><section class="series-more"><div class="series-section-head"><div><span class="series-section-label">BEHIND THE STORY</span><h2>More about ${escapeHTML(titleOf(s))}</h2></div></div>${mediaInfo(s, "tv")}</section></div></main>${footer()}`;
  bindCommon(); document.querySelector(".series-hero-copy")?.insertAdjacentHTML("beforeend", `<div class="series-social-actions">${ratingAction(s)}${shareAction()}</div>`); document.querySelector("#season-selector").onchange = async event => { state.selectedSeason = Number(event.currentTarget.value); await loadEpisodes(); render(); };
...
const PLAYER_ORIGINS = { "https://vidlink.pro":"vidlink", "https://vidsrc.sbs":"vidsrc", "https://www.vidking.net":"vidking", "https://www.2embed.online":"2embed" };
// Route a provider embed through SEVEN's adblocking player proxy (same-origin).
function proxyPlayerURL(url) { const entry = Object.entries(PLAYER_ORIGINS).find(([origin]) => url.startsWith(origin + "/")); return entry ? `/api/player/${entry[1]}/${url.slice(entry[0].length + 1)}` : url; }
function playerURL(item, progress = 0) {
  const preferences = currentPreferences(), provider = preferences.playerProvider || "vidlink";
  let url;
  if (provider === "2embed") url = item.type === "movie" ? `https://www.2embed.online/embed/movie/${item.id}` : `https://www.2embed.online/embed/tv/${item.id}/${item.season || 1}/${item.episode || 1}`;
  else if (provider === "vidsrc") {
    const base = item.type === "movie" ? `movie/${item.id}` : `tv/${item.id}/${item.season || 1}/${item.episode || 1}`;
    url = `https://vidsrc.sbs/embed/${base}?autoplay=1&color=b20710${progress > 0 ? `&t=${Math.floor(progress)}` : ""}`;
  }
  ...
  // Adblock on (default): serve the embed same-origin through the edge filter. Off: load the provider directly.
  return preferences.adblock !== false ? proxyPlayerURL(url) : url;
}
...
function renderPlayer() {
  ...
  const media = `<iframe class="player" src="${playerURL(p, party.syncPosition || 0)}" allow="autoplay; encrypted-media; fullscreen; picture-in-picture" allowfullscreen sandbox="allow-scripts allow-same-origin allow-forms"></iframe>`;
...
function profileSettingsDetail(profile, category, preferences, isNew) {
  ...
  if (category === "playback") return `<form id="profile-settings-form"><section class="profile-detail-card"><span class="brand">PLAYBACK</span><label class="profile-mobile-row profile-mobile-toggle"><i aria-hidden="true">▷</i><span><b>${t("Autoplay next episode")}</b><small>Continue a series automatically.</small></span><input name="autoplayNext" type="checkbox" ${preferences.autoplayNext !== false ? "checked" : ""}><em></em></label><label class="profile-mobile-row profile-mobile-toggle"><i aria-hidden="true">★</i><span><b>${t("Cinematic intro")}</b><small>Play the SEVEN ident when the app opens.</small></span><input name="introEnabled" type="checkbox" ${preferences.introEnabled !== false ? "checked" : ""}><em></em></label><label class="profile-mobile-row profile-mobile-toggle"><i aria-hidden="true">✦</i><span><b>${t("Autoplay spotlight")}</b><small>Rotate featured titles on Home.</small></span><input name="autoplayPreviews" type="checkbox" ${preferences.autoplayPreviews !== false ? "checked" : ""}><em></em></label><label class="profile-mobile-row profile-mobile-toggle"><i aria-hidden="true">◌</i><span><b>${t("New episode alerts")}</b><small>Get notified when favourites return.</small></span><input name="episodeAlerts" type="checkbox" ${preferences.episodeAlerts === true ? "checked" : ""}><em></em></label></section>${save}</form>`;
  if (category === "language") return `<form id="profile-settings-form"><section class="profile-detail-card"><span class="brand">LANGUAGE & DISPLAY</span><label class="profile-mobile-row profile-mobile-select"><i aria-hidden="true">A</i><span><b>${t("Display language")}</b><small>Choose the language used throughout SEVEN.</small></span><select name="language"><option ${preferences.language === "English" ? "selected" : ""}>English</option><option ${preferences.language === "Arabic" ? "selected" : ""}>Arabic</option><option ${preferences.language === "French" ? "selected" : ""}>French</option></select></label></section>${save}</form>`;
