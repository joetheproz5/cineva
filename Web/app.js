...
function header() { return `<header><button class="wordmark" data-home>CINEVA</button><label class="search"><span>⌕</span><input id="search" value="${escapeHTML(state.search)}" placeholder="Search movies and series" autocomplete="off"></label></header>`; }
...
function renderHome() {
  ...
  app.innerHTML = `${header()}<section class="hero" style="${f.backdrop_path ? `background-image:linear-gradient(90deg,#050505 4%,#05050588 55%,#050505cc),url(${TMDB_IMAGE}${f.backdrop_path})` : ""}"><span class="brand">CINEVA ORIGINAL EXPERIENCE</span><h1>${escapeHTML(titleOf(f))}</h1><p>${escapeHTML(f.overview || "")}</p><div><button class="primary" data-open="${f.type}:${f.id}">▶ Play</button><button class="secondary" data-open="${f.type}:${f.id}">Details</button></div></section>${state.error ? `<p class="setup">TMDB setup needed: ${escapeHTML(state.error)}. See README.</p>` : ""}<div id="rails">${Object.entries(state.catalog).map(([name, items]) => rail(name, items)).join("")}</div>`;
  bindCommon();
}
function rail(name, items) { return `<section class="rail"><h2>${name}</h2><div class="cards">${items.map(card).join("")}</div></section>`; }
function card(item) { return `<button class="card" data-open="${item.type}:${item.id}"><img src="${posterOf(item)}" alt="" loading="lazy"><b>${escapeHTML(titleOf(item))}</b><small>${yearOf(item) || "New"}</small></button>`; }
...
function renderSeries() {
  ...
  app.innerHTML = `${header()}<button class="back" data-home>‹ Home</button><section class="detail"><img src="${posterOf(s)}" alt=""><div><span class="brand">SERIES</span><h1>${escapeHTML(titleOf(s))}</h1><p>${escapeHTML(s.overview || "")}</p><button class="primary" data-play-series>▶ Play season ${state.selectedSeason}</button></div></section><section><h2>Episodes</h2><div class="season-row">${seasons.map(x => `<button class="season ${x.season_number === state.selectedSeason ? "active" : ""}" data-season="${x.season_number}">Season ${x.season_number}</button>`).join("")}</div><div id="episodes-list">${(state.episodes?.episodes || []).map(episode => `<button class="episode" data-play-episode="${episode.episode_number}"><img src="${episode.still_path ? TMDB_IMAGE + episode.still_path : "/icon.svg"}" alt=""><span><b>S${String(state.selectedSeason).padStart(2,"0")}E${String(episode.episode_number).padStart(2,"0")} · ${escapeHTML(episode.name)}</b><small>${escapeHTML(episode.overview || "No description available.")}</small></span></button>`).join("")}</div></section>`;
...
function renderSearch() { app.innerHTML = `${header()}<section><h2>Search results</h2>${state.searchResults.length ? `<div class="result-grid">${state.searchResults.map(card).join("")}</div>` : "<p>No matches found.</p>"}</section>`; bindCommon(); }
