...
function playerURL(item, progress = 0) {
  ...
  else if (provider === "dulo") url = item.type === "movie" ? `https://dulo.gg/embed/movie/${item.id}` : `https://dulo.gg/embed/tv/${item.id}/${item.season || 1}/${item.episode || 1}`;
...
function providerMenuHTML() {
  const labels = { vidlink:"VidLink", vidking:"Vidking", vidsrc:"VidSrc", "2embed":"2Embed", dulo:"Dulo" }, current = currentPreferences().playerProvider || "vidlink";
  return `<div class="provider-menu"><button class="provider-toggle" data-provider-menu>${labels[current] || "VidLink"} ▾</button><div class="provider-list" data-provider-list hidden>${["vidlink", "vidking", "vidsrc", "2embed", "dulo"].map(provider => `<button class="provider-option ${provider === current ? "active" : ""}" data-provider-select="${provider}">${labels[provider]}${provider === current ? " ✓" : ""}</button>`).join("")}</div></div>`;
}
