...
function header() {
  ...
  return `<header class="${settingsMode ? "settings-header" : `main-header ${state.route === "home" ? "home-header" : ""}`}"><button class="wordmark logo-only" data-home aria-label="SEVEN home"><img src="/assets/seven-wordmark-v2.png" alt="SEVEN"></button>${navigation}${search}${account}</header>`;
}
...
if ("serviceWorker" in navigator) {
  navigator.serviceWorker.register("/service-worker.js?v=197").catch(() => { /* The app keeps working from the network when registration fails. */ });
}
