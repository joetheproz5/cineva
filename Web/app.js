...
function playerURL(item, progress = 0) {
  ...
  if (preferences.playerProvider === "2embed") return item.type === "movie" ? `https://www.2embed.online/embed/movie/${item.id}` : `https://www.2embed.online/embed/tv/${item.id}/${item.season || 1}/${item.episode || 1}`;
...
function renderPlayer() {
  ...
  document.querySelector("[data-provider-toggle]")?.addEventListener("click", async () => { const next = currentPreferences().playerProvider === "2embed" ? "vidking" : "2embed"; updateCurrentPreferences({ playerProvider: next }); await saveAccount(); render(); });
  renderPartyPanel();
